...
class Config(object):
		...
		with self.graph.as_default():
			...
			with self.sess.as_default():
				...
				with tf.variable_scope("model", reuse=None, initializer = initializer):
					...
					self.train_op_single = self.optimizer.minimize(self.trainModel.loss)
	...
	def train_step(self, batch_h, batch_t, batch_r, batch_y, counter):
		...
		for i in range(self.sync_after):
			...
			_, c = self.sess.run([self.train_op_single, self.trainModel.loss], feed_dict = feed_dict)
			allreduce_loss += c
		#self.track_loss.append((counter, allreduce_loss))
		self.sess.run(self.barrier)

		if hvd.size() > 1:
			d = self.get_parameter_lists()
			for k, v in d.items():
				avg_tensor = hvd.allreduce(v)
				self.set_parameters_by_name(k, avg_tensor)
			...
			with self.sess.as_default():
				...
				if(hvd.rank() == 0):
					...
					if self.plot_train_loss:
						with open('./plot_loss/acc_16384/scaled_' + str(self.sync_after) + '.pkl', 'wb') as f:
