...
class Config(object):
		...
		with self.graph.as_default():
			...
			with self.sess.as_default():
				...
				with tf.variable_scope("model", reuse=None, initializer = initializer):
					...
					self.train_op_async = self.optimizer.minimize(self.trainModel.loss)
					self.dist_optimizer = hvd.DistributedOptimizer(self.optimizer, sparse_as_dense=False)
					self.train_op_dist = self.dist_optimizer.minimize(self.trainModel.loss)
					#Horovod end
				if(hvd.rank() == 0):
	...
	def train_step(self, batch_h, batch_t, batch_r, batch_y, counter):
		...
		_, loss = self.sess.run([self.train_op_dist, self.trainModel.loss], feed_dict)
		# threshold = counter - (self.ntrain_steps * 0.05)
		# if threshold > 0:
		# 	x = threshold%10
		...
		# 	_, loss = self.sess.run([self.train_op_dist, self.trainModel.loss], feed_dict)
		# elif(x<5):
		# 	_, loss = self.sess.run([self.train_op_async, self.trainModel.loss], feed_dict)
		# else:
		# 	_, loss = self.sess.run([self.train_op_dist, self.trainModel.loss], feed_dict)
	...
	def run(self):
		with self.graph.as_default():
			with self.sess.as_default():
				...
				self.ntrain_steps = self.train_times * self.nbatches
				counter = 0
